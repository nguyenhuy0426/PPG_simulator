...
void ParamController::setSpO2(float spo2) {
    PPGLimits limits = getPPGLimits(currentPPG.condition);
    PPGParameters pending = currentPPG;
    pending.spO2 = clamp(spo2, limits.spO2.min, limits.spO2.max);
    pendingPPG.hasPending = true;
    pendingPPG.pendingValue = pending;
    pendingPPG.requestTime = millis();
}

void ParamController::setRespRate(float rr) {
    PPGLimits limits = getPPGLimits(currentPPG.condition);
    PPGParameters pending = currentPPG;
    pending.respRate = clamp(rr, limits.respRate.min, limits.respRate.max);
    pendingPPG.hasPending = true;
    pendingPPG.pendingValue = pending;
    pendingPPG.requestTime = millis();
}
...
bool ParamController::validateParams(const PPGParameters& params) {
    ...
    if (params.spO2 < limits.spO2.min || params.spO2 > limits.spO2.max) return false;
    if (params.respRate < limits.respRate.min || params.respRate > limits.respRate.max) return false;
